...
public class Collision {
	//properties
	private boolean elasticity;//this defines whether collision elastic or inelastic
	private ArrayList<Ball> balls;
	JPanel size; //To get size of BallPanel
	
	//constructor
	public Collision (ArrayList<Ball> balls)
	{
		this.balls = balls;
		elasticity = true;
	}
	...
	public void collisionOfTwoBall(Ball ball1, Ball ball2)
	{
		if(doesTouch(ball1, ball2))
		{
			if(!elasticity)
			{
				double vx, vy;//x component of the velocity after the collision
				...
				ball1.setSpeed(vx, vy);
				ball2.setSpeed(vx, vy);
			}
			else
			{
				...
				vx1 = (v1 * Math.cos(theta1 - phi) * (m1 - m2) + 2*m2 * v2 * Math.cos(theta2- phi)) * Math.cos(phi)/(m1 + m2) + v1 * Math.sin(theta1 - phi)* Math.cos(Math.PI/2 + phi);
				vy1 = (v1 * Math.cos(theta1 - phi) * (m1 - m2) + 2*m2 * v2 * Math.cos(theta2- phi)) * Math.sin(phi)/(m1 + m2) + v1 * Math.sin(theta1 - phi)* Math.sin(Math.PI/2 + phi);
				vx2 = (v2 * Math.cos(theta2 - phi) * ( m2 - m1) + 2*m1 * v1 * Math.cos(theta1- phi)) * Math.cos(phi)/(m1 + m2) + v2 * Math.sin(theta2 - phi)* Math.cos(Math.PI/2 + phi);
				vy2 = (v2 * Math.cos(theta2 - phi) * ( m2 - m1) + 2*m1 * v1 * Math.cos(theta1- phi)) * Math.sin(phi)/(m1 + m2) + v2 * Math.sin(theta2 - phi)* Math.sin(Math.PI/2 + phi);
...
//				if(doesTouch(ball1, ball2))
//				{
//					ball1.updateLocation();ball2.updateLocation();
//				}
//				
//				if(doesTouch(ball1, ball2))
//				{
